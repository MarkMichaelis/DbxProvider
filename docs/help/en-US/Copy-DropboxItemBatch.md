...
Copies many Dropbox items in a single batched API call.

## SYNTAX

```
Copy-DropboxItemBatch [-FromPath] <String[]> [-ToPath] <String[]> [-DriveName <String>]
...
### Example 1
```powershell
PS> Copy-DropboxItemBatch -FromPath /a.txt,/b.txt -ToPath /backup/a.txt,/backup/b.txt
```

Copies two files in one request.

### Example 2
```powershell
PS> $src = Get-ChildItem Dbx:\Reports -File | ForEach-Object Path; $dst = $src | ForEach-Object { $_ -replace "/Reports/","/Archive/" }; Copy-DropboxItemBatch -FromPath $src -ToPath $dst
```

Copies an entire folder's files into ``/Archive`` in one batched call.
