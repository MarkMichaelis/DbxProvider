...
Locks one or more Dropbox files to prevent concurrent edits.

## SYNTAX

```
Lock-DropboxFile [-Path] <String[]> [-DriveName <String>] [-ProgressAction <ActionPreference>]
 [<CommonParameters>]
```
...
### Example 1
```powershell
PS> Lock-DropboxFile -Path /report.docx
```
...
### Example 2
```powershell
PS> "/a.txt","/b.txt" | Lock-DropboxFile
```

Locks multiple files via pipeline.
...
One or more Dropbox file paths to lock.
