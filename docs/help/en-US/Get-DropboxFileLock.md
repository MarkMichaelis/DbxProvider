...
Returns lock status for one or more Dropbox files.

## SYNTAX

```
Get-DropboxFileLock [-Path] <String[]> [-DriveName <String>] [-ProgressAction <ActionPreference>]
 [<CommonParameters>]
```
...
### Example 1
```powershell
PS> Get-DropboxFileLock -Path /report.docx
```

Shows lock state for one file.

### Example 2
```powershell
PS> Get-DropboxFileLock -Path /a.txt,/b.txt | Format-Table Path, IsLocked, LockHolder
```

Tabular view across multiple files.
...
One or more Dropbox file paths to query.
