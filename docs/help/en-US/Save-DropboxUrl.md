...
Asynchronously saves an external URL into Dropbox.

## SYNTAX

```
Save-DropboxUrl [-DropboxPath] <String> [-Url] <String> [-DriveName <String>]
...
### Example 1
```powershell
PS> Save-DropboxUrl -DropboxPath /Downloads/spec.pdf -Url "https://example.com/spec.pdf"
```

Queues a server-side fetch of an external URL into Dropbox.
...
Destination path inside Dropbox.
