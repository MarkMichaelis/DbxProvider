...
BeforeAll {
    ...
    $script:Folder = New-DbxTestFolder -TestName 'Export'
}
...
Describe 'Export-DropboxFile' -Skip:(-not $HasCredentials) {
    ...
    It 'attempts export for a plain file (skipped if no exportable doc available)' {
        # Export only works for Google Docs / Paper / similar exportable types.
        # We don't have one in /DbxProviderTests/, so skip with a clear message.
        Set-ItResult -Skipped -Because 'No exportable (Google Doc / Paper) document available in test workspace.'
    }
}
