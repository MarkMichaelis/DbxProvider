...
- [x] T1. Add `EnumerateItems(string startPath = "")` to MetadataCache: stream every
...
- [x] T2. Add `FindItems(Func<DropboxItem,bool> predicate, string startPath = "")`:
      EnumerateItems(startPath).Where(predicate), deduped by Path (OrdinalIgnoreCase).
- [x] TEST (Dbx.Core.UnitTests/MetadataCacheEnumerateTests.cs): after BuildAsync over
...
- [x] T3. New file src/IntelliTect.Dropbox.Core/ConflictMatch.cs: move `ConflictMatch`
...
- [x] T4. DELETE src/IntelliTect.Dropbox.Core/ConflictScanner.cs and
      test/Dbx.Core.UnitTests/ConflictScannerTests.cs.

### R2 Cross-cutting auto-refresh (DropboxCmdletBase)
- [x] T5. Add `protected MetadataCache GetRefreshedCache()` to DropboxCmdletBase:
...
- [x] T6. Add `protected static string StripDrivePrefix(string path)` to base (moved from
      ConflictCommands).

### R5 Find-DropboxItem cmdlet
- [x] T7. New cmdlet FindDropboxItemCommand [Find, "DropboxItem"], OutputType DropboxItem:
...
- [x] T8. Add 'Find-DropboxItem' to DbxProvider.psd1 CmdletsToExport.

### R1/R4 Rewrite Find-DropboxConflict
- [x] T9. Rewrite FindDropboxConflictCommand: keep OutputType ConflictMatch; params
...
- [x] T10. Migration: MigrateLegacyStateIfPresent(statePath) - if file exists and parses
...
### Tests (host)
- [x] T11. Extend FakeDropboxServiceClient with sync support (GetLatestCursorAsync ->
...
- [x] T12. Rewrite ConflictCmdletHostTests: InitializeCache + Build before
      Find-DropboxConflict; assert cached conflict returned.
- [x] T13. New FindDropboxItemHostTests: (a) scripted sync delta adds a conflict ->
...
- [x] T14. New migration host test: legacy .state.json archived to .bak, no error,
      cached conflict still returned.

### Docs / script
- [x] T15. Create docs/help/en-US/Find-DropboxItem.md (every declared param documented).
- [x] T16. Update docs/help/en-US/Find-DropboxConflict.md (remove -Full; fix -StatePath &
      DESCRIPTION to cache-backed reality; update SYNTAX).
- [x] T17. Rewrite Find-DropboxConflicts.ps1 docstrings honestly (cache-backed,
...
- [x] T18. Optional Pester Find-DropboxItem.Tests.ps1 mirroring Search-Dropbox.Tests.ps1
      (credential-gated).
